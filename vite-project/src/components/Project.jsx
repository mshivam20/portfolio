...
function ProjectCarousel() {
  ...
  return (
    <div id="projects" className="pt-10">
      <h1 className="text-white text-4xl font-bold text-center md:text-left md:ml-40 m-5 mb-10">Projects</h1>
      <section className="project relative w-full h-[500px] md:h-125 flex items-center justify-center overflow-hidden">

        {/* Left Arrow */}
        <button
          onClick={prevSlide}
          className="absolute left-2 md:left-10 z-20 bg-white/20 hover:bg-white/40 text-white p-3 rounded-full cursor-pointer"
        >
          ←
        </button>
        ...
        <button
          onClick={nextSlide}
          className="absolute right-2 md:right-10 z-20 bg-white/20 hover:bg-white/40 text-white p-3 rounded-full cursor-pointer"
        >
          →
        </button>

        {/* Cards */}
        <div className="relative w-full md:w-[800px] h-[450px] md:h-[420px]">
          ...
          {projects.map((project, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="bg-white rounded-2xl shadow-xl p-4 h-full flex flex-col text-left overflow-y-auto">
                  {project.image && (
                    <a href={project.url} className="shrink-0 block mb-4">
                      <img src={project.image} alt={project.title} className="w-full h-40 object-cover rounded-xl" />
                    </a>
                  )}
                  <a href={project.url} className="shrink-0 block mb-2">
                    <h2 className="text-xl font-bold hover:text-blue-800">{project.title}</h2>
                  </a>
                  <p className="text-gray-600 text-sm leading-relaxed pb-4">{project.description}</p>
                </div>
              </div>
            );
          })}
